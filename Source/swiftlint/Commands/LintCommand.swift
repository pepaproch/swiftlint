...
struct LintCommand: CommandProtocol {
    ...
    func run(_ options: LintOptions) -> Result<(), CommandantError<()>> {
        ...
        return configuration.visitLintableFiles(options: options, cache: cache) { linter in
            ...
            let noDuplicateViolations = self.removeDuplicates(violations: currentViolations)
            reporter.report(violations: noDuplicateViolations, realtimeCondition: true)
        }.flatMap { files in
            ...
            let noDuplicateViolations = self.removeDuplicates(violations: violations)
            
            reporter.report(violations: noDuplicateViolations, realtimeCondition: false)
            let numberOfSeriousViolations = noDuplicateViolations.filter({ $0.severity == .error }).count
            if !options.quiet {
                LintCommand.printStatus(violations: noDuplicateViolations, files: files,
    ...
    //--------------------------------------------------------------------------
    private func removeDuplicates(violations: [StyleViolation]) -> [StyleViolation]
    {
        var set = Set<String>()
        return violations.filter {
            let key = "\($0.location.file ?? String()):\($0.ruleDescription.name):\($0.location.line ?? 0):\($0.location.character ?? 0)"
